...
current = list(numbers.keys())[-1]

for index in range(len(numbers), 30000000):
    previous = current
    current = index - numbers.get(current) if current in numbers else 0
    numbers[previous] = index
...
Their question is: what will be the 2020th number spoken? In the example above, the 2020th number spoken will be 436.
