...
buses = [(int(bus), time) for time, bus in enumerate(input().split(",")) if bus != "x"]
little_m = [1] * len(buses)

big_m = 1
for index_buses, tup in enumerate(buses):
    if tup[1] > tup[0]:
        buses[index_buses] = (tup[0], tup[1] % tup[0])
    big_m *= tup[0]

    for index_prev, _ in enumerate(buses[:index_buses]):
        little_m[index_prev] *= tup[0]
    for index_post, _ in enumerate(buses[index_buses + 1:]):
        little_m[index_post + index_buses + 1] *= tup[0]
...
ys = []
for index_m, little_ms in enumerate(little_m):
    ys.append(get_y(little_ms % buses[index_m][0], buses[index_m][0]))

x = 0
for index_global, t in enumerate(buses):
    x += -t[1] * little_m[index_global] * ys[index_global]
...
Bus schedules are defined based on a timestamp that measures the number of minutes since some fixed reference point
 in the past. At timestamp 0, every bus simultaneously departed from the sea port. After that,
  each bus travels to the airport, then various other locations,
...
The time this loop takes a particular bus is also its ID number:
 the bus with ID 5 departs from the sea port at timestamps 0, 5, 10, 15, and so on.
  The bus with ID 11 departs at 0, 11, 22, 33, and so on. If you are there when the bus departs,
...
For example, in the list above, because bus ID 19 must depart seven minutes after the timestamp at which
 bus ID 7 departs, bus ID 7 will always also be departing with bus ID 19 at seven minutes after timestamp t.
...
1007153
29,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,37,x,x,x,x,x,433,x,x,x,x,x,x,x,x,x,x,x,x,13,17,x,x,x,x,19,x,x,x,23,x,x,
x,x,x,x,x,977,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,x,41
