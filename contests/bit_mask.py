...
while line != "":
    ...
    number = line.split(" = ")[1]
    if instruction == "mask":
        ones = [i for i, x in enumerate(reversed(number)) if x == "1"]
        floating = [i for i, x in enumerate(reversed(number)) if x == "X"]
    else:
        index = int(instruction.split("[")[1].split("]")[0])
        for index_one in ones:
            index |= (1 << index_one)

        floating_variations = list()
        floating_variations.append(index)

        for index_floating in floating:
            variations = len(floating_variations)
            for _ in range(variations):
                current = floating_variations.pop(0)
                floating_variations.append(current | (1 << index_floating))
                floating_variations.append(current & ~(1 << index_floating))

        for changed_index in floating_variations:
            mem[changed_index] = int(number)
...
After a brief inspection, you discover that the sea port's computer system uses a strange bitmask system
 in its initialization program. Although you don't have the correct decoder chip handy,
...
A floating bit is not connected to anything and instead fluctuates unpredictably. In practice,
 this means the floating bits will take on all possible values,
