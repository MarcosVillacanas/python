def evaluate():
    ...
    while index < len(exp):
        if exp[index] == "(":
            index += 1
            n = evaluate() if n == -1 else n + int(evaluate()) if symbol == "+" else n * int(evaluate())
        elif exp[index] == ")":
            return n
        elif exp[index] in "+*":
            symbol = exp[index]
        else:
            n = int(exp[index]) if n == -1 else n + int(exp[index]) if symbol == "+" else n * int(exp[index])
        index += 1
    return n
